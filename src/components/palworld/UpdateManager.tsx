...
export default function UpdateManager() {
  ...
  const loadInfo = useCallback(async () => {
    try {
      const res = await fetch(`/api/palworld-update?server=${serverId}`);
      if (res.ok) {
  ...
  const runUpdate = async () => {
    ...
    setUpdating(true);
    setMessage("Running SteamCMD update... This may take a few minutes.");
    setMessageType("info");
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-den-text-muted mt-1">
            Update Palworld dedicated server via SteamCMD
          </p>
        </div>
        <button
          onClick={loadInfo}
      ...
      <div className="bg-den-card border border-den-border rounded-xl p-5">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-den-text font-mono mt-1">{info?.steamAppId || 2394010}</p>
          </div>
          <div>
